...
export async function createI18nsheet({
  auth,
  title,
            ...
            data: [
              {
                rowData: [
                  ...(includeManual
                    ? generateUserManualHeaderRows(maxTranslationKeyLevel)
                    : []),
...
const generateExampleRows = (
  ...
  return exampleRows.flatMap((example) => {
    ...
    const rows = keyPath.flatMap((pathNode, index) =>
      lastPath[index] !== pathNode
        ? [
            {
              values: [
                // tag
...
const generateUserManualHeaderRows = (
  maxTranslationKeyLevel: number
): Schema$RowData[] => [
...
const fillCells = (count: number, cell: Schema$CellData) =>
