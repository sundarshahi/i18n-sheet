...
test("parseSheet", async () => {
  test("throws error on missing header row", async (t) => {
    ...
    await firstValueFrom(parseSheet({} as any).pipe(toArray()))
      .then(() => t.fail())
      .catch((error) => {
  ...
  test(
    "parses nested translations correctly",
    dataCaseTest(
      [
        ["###", ">>>", ">>>", ">>>", "en", "pl"],
  ...
  test(
    "fills missing language translations with empty string",
    dataCaseTest(
      [
        ["###", ">>>", "en", "pl"],
  ...
  test(
    "omits translations that are defined after 10 empty rows",
    dataCaseTest(
      [
        ["###", ">>>", "en", "pl"],
...
function dataCaseTest(
  spreadsheetRows: Row[],
  expectedOutput: TranslationEntry[]
): TestCase {
  return async (t) => {
    iteratorModule.spreadsheetRowsIterator = async function* () {
    ...
    const output = await firstValueFrom(parseSheet({} as any).pipe(toArray()));

    t.equal(output.length, expectedOutput.length, "Output length match");

    output.forEach((outputEntry, index) => {
      t.deepEqual(
        outputEntry,
        expectedOutput[index],
        `Translation entry #${index} match`
      );
    });
  };
}
